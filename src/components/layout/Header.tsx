        ...
        <nav className="hidden lg:flex items-center gap-1">
          {primaryNav.map((item) => (
            <Link key={item.path} to={item.path}
              className={cn("px-3 py-2 text-sm font-medium rounded-md transition-colors hover:bg-muted",
                location.pathname === item.path ? "text-primary bg-primary/5" : "text-muted-foreground"
              )}>{item.label}</Link>
          ))}
          <div className="relative" ref={moreRef}>
            <button onClick={() => setMoreOpen(!moreOpen)}
              className={cn("flex items-center gap-1 px-3 py-2 text-sm font-medium rounded-md transition-colors hover:bg-muted text-muted-foreground",
                moreNav.some((item) => location.pathname === item.path) && "text-primary bg-primary/5"
              )}>
              More<ChevronDown className={cn("h-4 w-4 transition-transform", moreOpen && "rotate-180")} />
            </button>
            {moreOpen && (
              <div className="absolute top-full right-0 mt-1 w-56 rounded-lg border border-border bg-popover p-1 shadow-lg animate-in fade-in-0 zoom-in-95">
                {moreNav.map((item) => (
                  <Link key={item.path} to={item.path} onClick={() => setMoreOpen(false)}
                    className={cn("block px-3 py-2 text-sm font-medium rounded-md transition-colors hover:bg-muted",
                      location.pathname === item.path ? "text-primary bg-primary/5" : "text-muted-foreground"
                    )}>{item.label}</Link>
                ))}
              </div>
            ...
            <SheetContent side="right" className="w-80 overflow-y-auto">
              <div className="flex flex-col gap-2 pt-8">
                <Button variant="ghost" size="icon" onClick={onSearchOpen} className="self-start mb-2">
                  <Search className="h-5 w-5" /><span className="ml-2">Search</span>
                </Button>
                ...
                <div className="border-t border-border mt-4 pt-4 flex flex-col gap-2">
                  {user ? (
                    <>
                      ...
                      <Button variant="ghost" onClick={async () => { await signOut(); setMobileOpen(false); }}>
                    ...
                    <>
                      ...
                      <Button className="gradient-bg text-white border-0" asChild>
                        <Link to="/sign-up" onClick={() => setMobileOpen(false)}>Get Started</Link>
                      </Button>
                    </>
                  )}
                </div>
