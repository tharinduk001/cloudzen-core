              ...
              return (
                <motion.div key={cat.name} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                  <Link to="/courses" className="block">
                    <Card className="text-center hover-glow hover:border-primary/30 transition-all group cursor-pointer h-full">
                  ...
                  <Card className="hover-glow hover:border-primary/30 transition-all group h-full">
                    <CardContent className="p-5">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <Badge variant={project.difficulty === "Easy" ? "secondary" : project.difficulty === "Medium" ? "default" : "destructive"} className="text-xs">{project.difficulty}</Badge>
                      </div>
        ...
        <div className="container">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {learningPaths.map((path, i) => (
              <motion.div key={path.id} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <Link to={`/learning-paths/${path.id}`}>
                  <Card className="hover-glow hover:border-primary/30 transition-all group h-full">
                    <CardContent className="p-5">
                      ...
                      <h3 className="font-display font-semibold mb-1 group-hover:text-primary transition-colors">{path.title}</h3>
      ...
      <section className="py-20 gradient-bg-subtle">
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {roadmaps.map((rm, i) => (
              <motion.div key={rm.id} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-center gap-1 mb-3">
                      {Array.from({ length: 5 }).map((_, j) => (
                        <Star key={j} className={`h-3.5 w-3.5 ${j < t.rating ? "text-yellow-500 fill-yellow-500" : "text-muted"}`} />
                      ))}
                    </div>
                ...
                <Link to={`/events/${event.id}`}>
                  <Card className="hover-glow hover:border-primary/30 transition-all group">
                    <CardContent className="p-5">
                      ...
                      <h3 className="font-display font-semibold mb-1 group-hover:text-primary transition-colors">{event.title}</h3>
