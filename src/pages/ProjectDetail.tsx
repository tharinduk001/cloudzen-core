      ...
      <div className="grid lg:grid-cols-[240px_1fr] gap-8">
        <aside className="hidden lg:block">
          <nav className="sticky top-24 space-y-1">
            {sidebarSections.map((s) => (
              <button key={s} onClick={() => setActive(s)} className={cn("block w-full text-left px-3 py-2 text-sm rounded-lg transition-colors", active === s ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>{s}</button>
            ))}
          </nav>
        </aside>
        <div className="space-y-8">
          ...
          <Card><CardContent className="p-6">
            ...
            <div className="aspect-video bg-muted rounded-lg flex items-center justify-center"><p className="text-muted-foreground">Diagram placeholder</p></div>
          </CardContent></Card>
          <div>
            <h2 className="font-display text-xl font-bold mb-4">Common Errors</h2>
            <Accordion type="multiple" className="space-y-2">
              {[{ q: "Permission denied error", a: "Try using sudo or check your IAM roles." }, { q: "Connection timeout", a: "Check your security group/firewall rules." }, { q: "Resource not found", a: "Verify the resource name and region." }].map((err, i) => (
          ...
          <Card className="gradient-bg-subtle"><CardContent className="p-6">
